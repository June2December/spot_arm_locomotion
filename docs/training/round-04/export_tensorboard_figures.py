#!/usr/bin/env python3
"""Export TensorBoard scalars from round-4c (stand, no gait) into PNG + JSON."""
...
RUN = Path(
    "/home/june/isaac_projects/spot_arm_locomotion/logs/rsl_rl/rough_spot_with_arm/2026-08-15_18-10-48"
)
FIG = ROOT / "figures"
DATA = ROOT / "data"
DT_POLICY = 0.02
...
def main() -> None:
    ...
    fig.suptitle("Round 4c PPO — 2026-08-15_18-10-48  (3000 iter, 4096 envs, plane)", fontsize=13)
    ...
    ax = axes[1, 0]
    ax.plot(x("Episode_Reward/track_lin_vel_xy_exp"), y("Episode_Reward/track_lin_vel_xy_exp"), color="#1d4ed8")
    ax.set_title("track_lin_vel_xy_exp")
    ax.set_xlabel("iteration")
    ...
    ax.set_title("Stands for the full 20 s timeout")
    ax.set_xlabel("iteration")
    ax.set_ylabel("seconds")
    ...
    fig, ax = plt.subplots(figsize=(8.2, 4.2))
    ax.plot(x("Metrics/base_velocity/error_vel_xy"), y("Metrics/base_velocity/error_vel_xy"), color="#7c3aed")
    ax.set_title("Velocity error stays ~ command size (robot speed ~ 0)")
    ax.set_xlabel("iteration")
    ax.set_ylabel("error_vel_xy")
    fig.savefig(FIG / "02-velocity-error.png")
    plt.close(fig)
    ...
    ax.set_title("Action std shrinks onto the stand pose")
    ax.set_xlabel("iteration")
    fig.savefig(FIG / "04-policy-std.png")
    plt.close(fig)
    ...
    (DATA / "round4c_metrics.json").write_text(json.dumps(metrics, indent=2))
    print("wrote", FIG)
