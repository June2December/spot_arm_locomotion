#!/usr/bin/env python3
"""Export TensorBoard scalars from round-2b training into PNG figures + JSON."""
...
RUN = Path(
    "/home/june/isaac_projects/spot_arm_locomotion/logs/rsl_rl/rough_spot_with_arm/2026-08-15_16-36-59"
)
FIG = ROOT / "figures"
DATA = ROOT / "data"
...
def main() -> None:
    ...
    fig.suptitle("Round 2b PPO — 2026-08-15_16-36-59  (1500 iter, 4096 envs, plane)", fontsize=13)
    ...
    ax.axhline(0.60, color="#9ca3af", ls="--", lw=1, label="stand-still expected ~0.60")
    ...
    ax.axhline(0.0, color="#9ca3af", ls=":", lw=1)
    ax.set_title("feet_air_time (never leaves the negative)")
    ax.set_xlabel("iteration")
    ax.set_ylabel("episode mean")
    ...
    ax.plot(x("Train/mean_episode_length"), steps * DT_POLICY, color="#1d4ed8")
    ax.axhline(20.0, color="#9ca3af", ls="--", lw=1, label="timeout 20 s")
    ax.set_title("Mean episode length reaches timeout (stands, does not fall)")
    ax.set_xlabel("iteration")
    ...
    fig, ax = plt.subplots(figsize=(8.2, 4.2))
    ax.stackplot(
        x("Episode_Termination/time_out"),
        y("Episode_Termination/time_out"),
        y("Episode_Termination/bad_orientation"),
        y("Episode_Termination/base_contact"),
        labels=["time_out", "bad_orientation", "base_contact"],
        colors=["#bbf7d0", "#fecaca", "#fed7aa"],
    )
    ax.plot(x("Episode_Termination/time_out"), y("Episode_Termination/time_out"), color="#15803d", lw=1.2)
    ax.set_title("Most episodes now time out at 20 s")
    ax.set_xlabel("iteration")
    ...
    ax.set_title("Mean reward stays negative — standing penalties, no gait return")
    ax.set_xlabel("iteration")
    ...
    ax.axhline(0.0, color="#9ca3af", ls=":", lw=1)
    ax.set_title("Per-term episode rewards")
    ax.set_xlabel("iteration")
    ax.set_ylabel("episode mean")
    ...
    ax.axhline(0.344, color="#9ca3af", ls="--", lw=1, label="stand-still E[|cmd_xy|] ~0.34")
    ax.set_title("Command tracking error matches standing still")
    ax.set_xlabel("iteration")
    ax.set_ylabel("error")
    ax.legend()
    ...
    fig, ax = plt.subplots(figsize=(8.2, 4.2))
    ax.plot(x("Metrics/success_rate"), y("Metrics/success_rate"), color="#0f766e")
    ax.set_title("success_rate is high because the threshold is 0.5 m/s")
    ax.set_xlabel("iteration")
    ax.set_ylabel("success_rate")
    ax.set_ylim(0, 1.05)
    fig.savefig(FIG / "06-success-rate.png")
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8.2, 4.2))
    ax.plot(x("Policy/mean_std"), y("Policy/mean_std"), color="#374151")
    ax.set_title("Action std stays around 0.3–0.5 (stand policy, not a gait)")
    ax.set_xlabel("iteration")
    ax.set_ylabel("mean std")
    ...
    it = downsample(x("Train/mean_reward"), 21)
    metrics = {
        "run": str(RUN),
        ...
        "highlights": {
            ...
            "feet_air_time_last": last["Episode_Reward/feet_air_time"],
            "error_vel_xy_last": last["Metrics/base_velocity/error_vel_xy"],
            "success_rate_last": last["Metrics/success_rate"],
            "stand_still_expected_track_lin": 0.602,
        },
        "canvas_series": {
            "iteration": [str(int(v)) for v in it],
            "episode_length_s": downsample(y("Train/mean_episode_length") * DT_POLICY, 21),
            "mean_reward": downsample(y("Train/mean_reward"), 21),
            "bad_orientation": downsample(y("Episode_Termination/bad_orientation"), 21),
            "base_contact": downsample(y("Episode_Termination/base_contact"), 21),
            "time_out": downsample(y("Episode_Termination/time_out"), 21),
            "track_lin_xy": downsample(y("Episode_Reward/track_lin_vel_xy_exp"), 21),
            "track_ang_z": downsample(y("Episode_Reward/track_ang_vel_z_exp"), 21),
            ...
            "success_rate": downsample(y("Metrics/success_rate"), 21),
            "mean_std": downsample(y("Policy/mean_std"), 21),
        },
    }
    (DATA / "round2b_metrics.json").write_text(json.dumps(metrics, indent=2))
    print("wrote figures to", FIG)
    print("episode_length_s_last", metrics["highlights"]["episode_length_s_last"])
    print("track_lin_vel_last", metrics["highlights"]["track_lin_vel_last"])
    print("canvas iteration", metrics["canvas_series"]["iteration"])
